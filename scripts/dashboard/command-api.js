...
function registerCommandAPI(app, pool = null) {
  ...
  app.post('/api/command/content/new', async (req, res) => {
    const { platform = 'linkedin', post_type = 'draft', topic, content_text, adapter = 'yohann' } = req.body;
    try {
      if (!content_text) return res.status(400).json({ error: 'content_text required' });
      const result = await query(`
        INSERT INTO content_posts (platform, post_type, topic, content_text, adapter, status)
